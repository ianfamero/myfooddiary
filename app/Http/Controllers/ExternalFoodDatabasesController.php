...
use App\FoodList;
use Auth;
use App\Http\Requests\ExternalFoodDatabasesRequest;

class ExternalFoodDatabasesController extends Controller
{
    public function search(ExternalFoodDatabasesRequest $request) {
        $input = $request->all();
        if($input['search_query'] == "") {
          return [];
        }
        if($input['food_database'] == 0) {
          $url = "https://api.nal.usda.gov/ndb/search/?format=json&ds=Standard%20Reference&q=".urlencode($input['search_query'])."&sort=n&max=25&offset=0&api_key=OAnc7ObDIUOiBHfRBdJLegY41PPEKroYUoGxqNyw";
        ...
        else if($input['food_database'] == 1) {
          $url = "https://api.nutritionix.com/v1_1/search/".urlencode($input['search_query'])."?results=0:50&fields=item_name,brand_name,nf_calories,nf_total_fat,nf_total_carbohydrate,nf_protein,nf_serving_size_qty,nf_serving_size_unit&appId=4decda58&appKey=9668aaaea93d81038720d848fa1065ed";
